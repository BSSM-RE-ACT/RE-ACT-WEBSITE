...
Contact 섹션의 폼(이름/이메일/메시지)을 제출하면 백엔드가 SMTP로 이메일을 보내요. `mailto:` 링크와
달리 방문자 기기에 메일 앱이 설정되어 있지 않아도 항상 동작합니다. 스팸 방지로 허니팟 필드와
IP당 30초 쿨다운이 들어가 있어요.

### 문의 메일 켜는 방법 (직접 하셔야 해요)

Gmail 계정으로 보내는 게 제일 간단하고 무료예요. Gmail은 SMTP 로그인에 일반 비밀번호를 쓸 수
없고 **앱 비밀번호**가 필요해서, 여러분의 구글 계정으로 직접 발급받아야 해요.

1. 보내는 용도로 쓸 구글 계정(동아리 전용 Gmail 추천)에서 **2단계 인증을 먼저 켜세요**
   ([myaccount.google.com/security](https://myaccount.google.com/security))
2. 2단계 인증을 켠 뒤 [myaccount.google.com/apppasswords](https://myaccount.google.com/apppasswords)
   에서 앱 비밀번호를 새로 만드세요 (앱 이름은 아무거나, 예: "RE:ACT 웹사이트")
3. 발급된 16자리 비밀번호(공백 없이)를 복사해서:
   - Docker 배포: 루트 `.env`의 `SMTP_USERNAME`에 그 Gmail 주소, `SMTP_PASSWORD`에 앱 비밀번호
   - 로컬 개발: `backend/.env`에 동일하게
4. `CONTACT_TO_EMAIL`에 문의를 받을 주소를 넣으세요 (비워두면 `SMTP_USERNAME`으로 옵니다 — 같은
   주소로 보내고 받아도 괜찮아요)
5. 백엔드를 재시작하면 바로 켜져요.

설정 전까지는 폼 제출 시 "문의 메일 발송이 아직 설정되지 않았어요" 에러가 나요 — 사이트가 죽는 게
아니라 이 기능만 꺼져 있는 상태입니다.
